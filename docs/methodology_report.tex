...
the wrong quantity to judge: delivered carbon spans 9.9$\times$ wet-to-dry
because the dissolution response to $X$ is nonlinear, and with the drainage ceiling
of \S\ref{sec:ceiling} applied it spans 125$\times$ -- against a
141$\times$ contrast in $q$ itself.

More decisively, the capped contrast is \textbf{invariant to $D_w$}:
125.3$\times$ at $D_w = 0.001$ against 124.8$\times$ at
0.03, three orders of magnitude apart. The ceiling is linear in
$q$ and binds on 91\% of cropland area, so it has already taken the
...
Uncapped, delivered carbon spans 9.9$\times$ from the wettest to the driest
5\% of cropland; with the ceiling applied it spans 125$\times$, close to
the 141$\times$ contrast in the drainage data. The larger figure is the
