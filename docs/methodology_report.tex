...
$\Psi = 0.2950$\,t\,\cotwo{}\,t$^{-1}$ the feedstock's stoichiometric
...
1.9099$\,\textmu m dissolves 12\% of the reference
...
weathered fell from 0.02\% to 0.00\% of cropland.
...
the wrong quantity to judge: delivered carbon spans 13.0$\times$ wet-to-dry
because the dissolution response to $X$ is nonlinear, and with the drainage ceiling
of \S\ref{sec:ceiling} applied it spans 110$\times$ -- against a
141$\times$ contrast in $q$ itself.

More decisively, the capped contrast is \textbf{invariant to $D_w$}:
111.0$\times$ at $D_w = 0.001$ against 110.0$\times$ at
0.03, three orders of magnitude apart. The ceiling is linear in
$q$ and binds on 77\% of cropland area, so it has already taken the
...
with topsoil rock. Treat $q_r$ and $q_{tot}$ as a bracket: 1.29 against
1.11\,Gt\,\cotwo{}\,yr$^{-1}$ unbounded, reported by gate 2d on every
build.
...
\texttt{delivered\_basalt} & 9.2 & 6.9 & 0.295 & $\bullet$ \\
\bottomrule
\end{tabular}
\end{center}

At $A = 30$\,t\,ha$^{-1}$ this caps gross removal at
$A\Psi = 8.85$\,t\,\cotwo{}\,ha$^{-1}$\,yr$^{-1}$, which is the top
...
off, the reported \cotwo{} exceeds this bound on 77.0\% of cropland
area by a median 1.8$\times$, and should then be read as an upper limit on
...
3.66\,mmol\,L$^{-1}$ at $\Omega = 1$ and 5.00 at
$\Omega = 3.16228$, the shipped value -- justified because
...
Applying it takes the global total from 1.29 to
0.63\,Gt\,\cotwo{}\,yr$^{-1}$. The consequence that matters is not the
...
at the US rate, $d_0 = 50$ km is \$5.00/t). $r(x)$ is a
regional rate rasterised from Natural Earth countries
(US/Canada \$0.1; Europe \$0.09; Brazil/Latin America \$0.055; India/South Asia \$0.045; China/SE Asia \$0.07; Africa \$0.11; elsewhere \$0.08), sourced in
...
\$0.12/t-km, was right for the US and roughly twice too high
...
weathers 7.7\% in year one and 46.1\% by year ten, so
one application delivers $5.9\times$ its year-one carbon over a
...
$k \approx 5.9$ years. A field on a longer rotation removes less than
the map shows.
...
$L_1 = \log_{10}(R/R_{\mathrm{ref}})$ & -0.58 & -0.10 & +0.60 \\
$\eta_{\mathrm{DIC}}$ & 0.937 & 0.997 & 0.999 \\
$\eta_{\mathrm{tr}}$ & 0.507 & 0.856 & 0.953 \\
Drainage $q$ (m\,yr$^{-1}$) & 0.031 & 0.178 & 0.613 \\
Fraction weathered, year 1 & 2.3\% & 7.7\% & 33.4\% \\
Gross removal (t\,\cotwo{}\,ha$^{-1}$yr$^{-1}$) & 0.20 & 0.66 & 2.55 \\
\bottomrule
\end{tabular}
\end{center}

Global gross removal is 1.29\,Gt\,\cotwo{}\,yr$^{-1}$ unbounded and
0.63 with the drainage ceiling applied. Neither is a deployment forecast:
...
\item \textbf{The drainage ceiling binds on 77.0\% of cropland area},
...
transport term.} Uncapped, delivered carbon spans 13.0$\times$ from the
wettest to the driest 5\% of cropland; with the ceiling applied it spans
110$\times$, close to the 141$\times$ contrast in the drainage
