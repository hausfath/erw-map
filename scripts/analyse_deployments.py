...
def regime_comparison(rows: list[dict]) -> None:
    ...
    print("    - grain size spans 67-500 um and is unresolved per deployment;")
    print("      that alone can span 7.5x geometric SSA, and up to 33x once")
    print("      distribution width is allowed for. It is larger than every")
    print("      regime difference here")
...
def why_year_one_fw_is_the_wrong_observable(rows: list[dict]) -> None:
    ...
    print("      while grain size alone could plausibly account for 7.5-33x.")
    print()
...
def what_would_make_this_a_real_test(rows: list[dict]) -> None:
    ...
    print("     feedstocks with identical d80 can differ 33x in reactive area.")
