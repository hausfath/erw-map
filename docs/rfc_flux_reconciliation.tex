...
9--60$\times$ \emph{below} the ceiling, which matters for \S\ref{sec:notexplain}.
...
(\S\ref{sec:drainage}) moved it to 1.41$\times$. The direction of the effect is
...
0.11--0.75\,mmol\,L$^{-1}$,
i.e.\ 9--60$\times$ \emph{below} this bound, because cations are retained in secondary
...
reports on every run that 93.0\% of cropland area exceeds the bound by a
median 2.9$\times$, so the finding does not disappear with the cap.

On external consistency: with the ceiling \emph{off} our global total is
2.49\,Gt\,\cotwo{}\,yr$^{-1}$ and with it \emph{on} 0.91, both inside
