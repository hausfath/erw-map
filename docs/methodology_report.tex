...
weathered fell from 2.15\% to 0.77\% of cropland.
...
with topsoil rock. Treat $q_r$ and $q_{tot}$ as a bracket: 2.49 against
2.15\,Gt\,\cotwo{}\,yr$^{-1}$ unbounded, reported by gate 2d on every
build.
...
93.0\% of cropland area by a median 2.9$\times$, so it should be
...
Applying it takes the global total from 2.49 to
0.91\,Gt\,\cotwo{}\,yr$^{-1}$. The consequence that matters is not the
...
weathers 18.7\% in year one and 78.1\% by year ten, so
one application delivers $4.0\times$ its year-one carbon over a
...
$k \approx 4.0$ years. A field on a longer rotation removes less than
the map shows.
...
$L_1 = \log_{10}(R/R_{\mathrm{ref}})$ & -0.57 & -0.04 & +0.64 \\
$\eta_{\mathrm{DIC}}$ & 0.925 & 0.997 & 0.999 \\
$\eta_{\mathrm{tr}}$ & 0.507 & 0.856 & 0.953 \\
Drainage $q$ (m\,yr$^{-1}$) & 0.031 & 0.178 & 0.613 \\
Fraction weathered, year 1 & 5.6\% & 18.7\% & 61.8\% \\
Gross removal (t\,\cotwo{}\,ha$^{-1}$yr$^{-1}$) & 0.48 & 1.59 & 4.60 \\
\bottomrule
\end{tabular}
\end{center}

Global gross removal is 2.49\,Gt\,\cotwo{}\,yr$^{-1}$ unbounded and
0.91 with the drainage ceiling applied. Neither is a deployment forecast:
